...
<!-- DataTables -->	
	...
	<script type="text/javascript">
		...
		function details(oid) {
			...
			window.location.href = '../orderdetail/details.controller';
		}
			...
			const result = Swal.fire({
			    ...
			    cancelButtonText : '取消', 
				  ...
				  if (result.value) {	
					$.ajax({
						type:'post',
						url:'/order/orderdeletebyid.controller?oid='+ oid,
					    dataType:'JSON',
					    contentType:'application/json',
					    complete: function(){
				    	window.location.href = '/order/orderrecord.controller';
					    }
					});
				}
  	    	})
			...
			if(user == null || user.length==0 ){
				...
				success: function(data){
					...
					if(data==null || data.length==0){
						...
						table.append("<thead><tr><th>訂單編號</th><th>" +
								"訂單帳戶</th><th>訂單日期</th><th>" +
								"商品總數</th><th>商品明細</th><th>功能列表</th></tr></thead>");
						...
						$.each(data, function(i,n){
		  				  var tr = "<tr>" + "<td align='center'>" + n.oid + "</td>" +
		  	    			       ...
		  	    			       "<td align='center'>" +
		  	    			       "<button style='border-width:0.01px;border-radius:0.3em;color:white;background-color:#0066FF	' type='button' value='查詢' onclick='details(" + n.oid + ")'>查詢</button>" + 
		  	    			       "</td>" +
		  	    			       "<td align='center'>" +
		  	    			       "<button style='border-width:0.01px;border-radius:0.3em;color:white;background-color:#E00000	'  type='button' value='刪除' onclick='del(" + n.oid + ")'>刪除</button>" + 
		  	    			       "</td>" +
		  				           "</tr>";
		  				   t += tr
		  				});
...
<body>
	...
	<h2 class="text-center" style="color:#800000;">訂單系統</h2>
	<hr>
	<div style="margin:auto;background:#F5F5F5;border-width:4.5px;border-style:outset;border-color:#880000;padding:55px;border-radius:3em">
