<%@page import="tw.fooddemo.model.FoodBeanStock"%>
...
<body>
	...
	<div class="wrapper d-flex align-items-stretch">
		...
		<div id="content" class="p-4 p-md-5">
			...
			<!-- 大家的內容 START-->
			<h2 class="text-center" style="color:#800000;">食材庫存資料系統</h2>
	<hr>
	<div class="text-center" style="width:1400px;margin:auto;background:#F5F5F5;border-width:4.5px;border-style:outset;border-color:#880000;padding:55px;border-radius:3em">
		
<form  method="post" action="./foodstockselectall.controller" id="From" >
...
<div>
			...
			ArrayList<FoodBeanStock> List = (ArrayList<FoodBeanStock>) request.getAttribute("reg_food");

			for (FoodBeanStock f : List) {
			%>
					<tr>
						<td><%=f.getId()%></td>
						<td><%=f.getDate()%></td>
						<td><%=f.getBos()%></td>
						<td><%=f.getGreenonion()%></td>
						<td><%=f.getGarlic()%></td>
						<td><%=f.getEgg()%></td>
						<td><%=f.getKelp()%></td>
						<td><%=f.getSeasoningpack()%></td>
						<td><%=f.getAlkalinewater()%></td>
						<td><%=f.getMiso()%></td>
						<td><%=f.getSalt()%></td>
						<td><%=f.getSesame()%></td>
						<td><%=f.getMushroom()%></td>
						<td><%=f.getOnion()%></td>
						<td><%=f.getChinesecabbage()%></td>
						<td><%=f.getBambooShoots()%></td>
						<td><%=f.getNoodle()%></td>
						<td><%=f.getBeefBone()%></td>
						<td><%=f.getPork()%></td>
						<td><%=f.getBeef()%></td>
